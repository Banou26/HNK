...
const makeMethod = name => (_this, ...args) => ({
  ...shadyCssParser.NodeFactory.prototype[name].apply(_this, args),
...
const flattenRoutes = ({routes, path = '', parent, map = new Map()}) =>
  routes.forEach(route => {
    ...
    const obj = {...childPath, ...parent && {parent}, keys, regex: pathToRegexp__default(childPath, [], pathToRegexpOptions), toPath: pathToRegexp.compile(childPath)};
